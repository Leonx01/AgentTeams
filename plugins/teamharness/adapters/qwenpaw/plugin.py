...
def _runtime_text() -> str:
    path = Path(os.getenv("TEAMHARNESS_RUNTIME_CONFIG", "/etc/agentteams/runtime.yaml"))
    try:
...
def _team_prompt(_agent: Any) -> str:
    base = ASSET_DIR / "prompts" / "team" / "TEAMS.md"
    try:
        prompt = base.read_text(encoding="utf-8").strip()
    except OSError:
        prompt = ""
    runtime = _runtime_text().strip()
    if runtime:
        prompt = f"{prompt}\n\n## AgentTeams runtime contract\n\n```yaml\n{runtime}\n```"
    return prompt
    ...
    if isinstance(value, dict):
        for key, item in list(value.items()):
            if isinstance(item, str):
                for rule in rules:
                    item = re.sub(re.escape(rule), "[REDACTED]", item, flags=re.IGNORECASE)
                value[key] = item
            else:
                _sanitize(item)
        return
...
def _load_trace_module() -> Any | None:
    ...
    if not path.is_file():
        return None
    spec = importlib.util.spec_from_file_location("agentteams_teamharness_task_trace", path)
...
def _room_id(request: Any) -> str:
    data = request.model_dump() if hasattr(request, "model_dump") else vars(request) if hasattr(request, "__dict__") else {}
...
class TeamHarnessPlugin:
    ...
    def _register_http(self, api: Any) -> None:
        ...
        def sync_endpoint() -> dict[str, Any]:
            return {"ok": True, "plugin": "teamharness", "managedBy": "qwenpaw-plugin-api"}
