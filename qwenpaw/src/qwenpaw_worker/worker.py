...
class Worker:
    def __init__(self, config: WorkerConfig) -> None:
        ...
        self.api_client = QwenPawApiClient(f"http://127.0.0.1:{config.console_port}")
    ...
    async def start(self) -> bool:
        ...
        logger.info("qwenpaw worker preparation complete component=worker worker=%s", self.config.worker_name)
        return True
    ...
    def _apply_runtime_adapter(self) -> None:
        self._prepare_default_plugins()
        self._configure_builtin_plugin_mcp()
    ...
    async def _run_qwenpaw(self) -> None:
        ...
        try:
            ...
            await asyncio.to_thread(self._configure_builtin_plugin_mcp)
            ...
            if self._process.returncode is not None:
                ...
                logger.warning(
                    ...
                    "returncode=%s stopping=False duration_ms=0",
                    self.config.worker_name,
                    returncode,
                )
                return
    ...
    def _configure_builtin_plugin_mcp(self) -> None:
